/*
  ...
  Provide access to "registers" via i2c.  The registers can vary in
  width, but the offset must be between 0 and 127 (unsigned char with
  bit 7 being the read/write bit).  See access.c for an example of
  accessing the registers from the Pi.

  The important and interesting bits were taken from
  https://github.com/eriksl/usitwislave.git, which has the following
  license.
