...
struct tr trs[] = {
	...
	{"voltage1", trvoltage1, 2, 1},
	{"voltage2", trvoltage2, 2, 1}
};
	...
	*/
	...
	while (0 < test--) {
		...
		*/
		...
		if (EXIT_SUCCESS != traccess(pi, trvoltage1, 1, &voltage1) ||
		    EXIT_SUCCESS != traccess(pi, trvoltage2, 1, &voltage2)) {
			fprintf(stderr, "Read Failed\n");
			rc = EXIT_FAILURE;
			goto exit;
		}

		printf("Voltage1 0x%x Voltage2 0x%x\n", voltage1, voltage2);
	}
