...
usage(const char *prog, int exit_code)
{
	...
	printf("  -a --ipaddress  Hostname or IP Address (default localhost)\n"
	       ...
	       "  -B --ocr1b      Set the OCR1B register to the given value\n"
...
main(int argc, char *argv[])
{
	...
	*/
	...
	while (1) {
		...
		c = getopt_long(argc, argv, "a:A:B:c:d:hi:p:s:t:v",
				lopts, NULL);
		...
		case 'a':
			...
			break;
		case 'A':
			ocr1a = (unsigned short)strtoul(optarg, NULL, 0);
			write_ocr1a = 1;
			break;
		case 'B':
			ocr1b = (unsigned short)strtoul(optarg, NULL, 0);
			write_ocr1b = 1;
			break;
		case 'c':
	...
	*/
	...
	while (0 < test--) {
		...
		*/
		...
		if (0 != write_ocr1b) {
			if (EXIT_SUCCESS != traccess(pi, trocr1b, 0, &ocr1b)) {
				fprintf(stderr, "Read Failed\n");
				rc = EXIT_FAILURE;
				goto exit;
			}
		...
		*/
		...
		if (EXIT_SUCCESS != traccess(pi, trocr1a, 1, &ocr1a) ||
		    EXIT_SUCCESS != traccess(pi, trocr1b, 1, &ocr1b)) {
			fprintf(stderr, "Read Failed\n");
			rc = EXIT_FAILURE;
			goto exit;
		}

		printf("OCR1A=0x%x OCR1B=0x%x\n", ocr1a, ocr1b);
	}
